...
class MY_Controller extends CI_Controller
{
	public function __construct()
	{
        ...
        $this->load->model(array('menu_u','common/prefs_model'));
...
class Bank_Controller extends MY_Controller
{
        public function __construct()
        {
            ...
            else
            {
                ...
                $this->load->library('admin/page_title');
                $this->load->library('admin/breadcrumbs');
                $this->load->model('bank/core_model');
                $this->load->helper('menu');
                $this->lang->load(array('admin/main_header', 'admin/main_sidebar', 'admin/footer', 'admin/actions'));

                /* Load library function  */
                $this->breadcrumbs->unshift(0, $this->lang->line('menu_dashboard'), 'bank/dashboard');
...
class Notary_Controller extends MY_Controller
{
        public function __construct()
        {
            parent::__construct();

            if ( ! $this->ion_auth->logged_in() OR ! $this->ion_auth->in_group("Notaris"))
            {
                redirect('auth/login', 'refresh');
            }
            else
            {
                ...
                $this->load->library('admin/page_title');
                $this->load->library('admin/breadcrumbs');
                $this->load->model('notary/core_model');
                $this->load->helper('menu');
                $this->lang->load(array('admin/main_header', 'admin/main_sidebar', 'admin/footer', 'admin/actions'));

                /* Load library function  */
                $this->breadcrumbs->unshift(0, $this->lang->line('menu_dashboard'), 'notary/dashboard');
